...
class Device {
  ...
  /**
     ...
     */
  requestDeviceStatus (device, mac) {
  ...
  _handleResponse (msg, rinfo) {
    ...
    console.log('[UDP] decrypt message %s: %s, %s', pack.t, message, JSON.stringify(pack))

    // If package type is response to handshake
    if (pack.t === 'dev') {
      this._setDevice(message.cid, pack.name, pack.mac, rinfo.address, rinfo.port)
    ...
    console.log('[UDP] Unknown message of type %s: %s, %s', pack.t, message, JSON.stringify(pack))
  }
